...
const validationSchema = createRegisterBalanceSchema.extend({
  employee_id: z.string().min(1, "El cuadre de caja necesita un cajero"),
});

type CreateRegisterBalance = z.infer<typeof validationSchema>;
...
export default function CreateRegisterBalancePage() {
  ...
  const [expenses, setExpenses] = useState<ExpenseProxy[]>([]);

  const [isLoading, setIsLoading] = useState(false);
  const router = useBranchRouter();

  const onSubmit: SubmitHandler<CreateRegisterBalance> = async (data) => {
    console.log(data);
  ...
  function handleAddExpense(data: Omit<ExpenseProxy, "id">) {
    setLastIndex((e) => e + 1);

    expenses.push({ ...data, id: lastIndex });
  }

  function handleDeleteExpense(id: number) {
    const newExpences = [...expenses];
    const expenseIndex = newExpences.findIndex((e) => e.id === id);

    newExpences.splice(expenseIndex, 1);
                ...
                {expenses.map((expense) => (
                  ...
                  >
                    ...
                    <div className="gap-2 flex items-center">
                      <p className="text-sm">
                        COP {moneyFormatter.format(expense.amount)}
                      </p>
                      <Button
                        isIconOnly
                        variant="flat"
